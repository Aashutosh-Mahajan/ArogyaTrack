...
function PatientDashboard(): React.JSX.Element {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Current Diseases & Vitals Status */}
      <CurrentDiseases />
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{t('active_prescriptions_title')}</CardTitle>
            <Link
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-gray-900">
                      {t('prescription')} #{prescription.prescription_number}
                    </p>
                    <p className="text-sm text-gray-600 mt-1">
                      {prescription.medicines.length} {t('medicines_count')}
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
                      {t('issued')}: {prescription.issued_at ? new Date(prescription.issued_at).toLocaleDateString(locale) : 'N/A'}
                    </p>
                  </div>
                  <div>
