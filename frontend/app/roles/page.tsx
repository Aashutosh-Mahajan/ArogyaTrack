...
function UserIcon() {
  return (
    <svg width="32" height="32" viewBox="0 0 256 256" fill="#5ca88c">
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          width: 800,
          height: 600,
          background: 'radial-gradient(circle, rgba(92,168,140,0.12) 0%, transparent 60%)',
        }}
      />
      ...
      <div className="relative z-10 pt-10 flex justify-center mb-12 animate-[fadeUpRole_0.6s_ease-out_forwards] opacity-0">
        <Link href="/" className="flex flex-col items-center gap-1 group">
          <div
            ...
            style={{ background: 'linear-gradient(135deg, #4a8a6f, #3a6f58)' }}
          >
          ...
          <span className="text-[0.6rem] font-bold tracking-[0.05em] uppercase" style={{ color: '#5ca88c' }}>
            GOVT. OF INDIA
          </span>
        </Link>
      </div>
        ...
        {roles.map((role, i) => {
          ...
          return (
            <Link
              key={role.title}
              href={role.href}
              className="group relative flex flex-col w-[240px] max-[768px]:w-full max-[768px]:mb-3 rounded-[1.75rem] p-1.5 cursor-pointer transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-2"
              style={{
                backgroundColor: 'rgba(92,168,140,0.05)',
                border: '1px solid rgba(92,168,140,0.15)',
                animation: `fadeUpRole 0.6s ease-out ${0.1 * (i + 1)}s forwards`,
                opacity: 0,
              }}
            >
              <div
                className="relative flex flex-col flex-1 rounded-[1.4rem] p-7 pb-6 transition-colors duration-300"
                style={{
                  backgroundColor: '#211c16',
                  boxShadow: 'inset 0 1px 1px rgba(255,255,255,0.06)',
                }}
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#2b241c'; }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#211c16'; }}
              >
                {/* Badge */}
                {role.badge && (
                  <span
                ...
                {/* Icon */}
                <div
                  className="w-16 h-16 rounded-full flex items-center justify-center mb-6 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:scale-105"
                  style={{ backgroundColor: 'rgba(92,168,140,0.15)' }}
                >
                  <IconComp />
                </div>
                ...
                <div className="flex items-center justify-between mt-6">
                  ...
                  <span
                    className="btn-icon-wrap w-7 h-7 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover:translate-x-1"
                    style={{ color: '#5ca88c', backgroundColor: 'rgba(92,168,140,0.15)' }}
                  >
                    <ArrowRightIcon />
                  </span>
                </div>
      ...
      <style jsx>{`
        ...
        @keyframes borderPulse {
          from { border-color: rgba(92,168,140,0.15); box-shadow: 0 0 0 transparent; }
          to { border-color: rgba(92,168,140,0.3); box-shadow: 0 0 15px rgba(92,168,140,0.05); }
        }
      `}</style>
    </div>
  );
